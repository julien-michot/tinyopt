...
/// Return current residuals with the jacobian d f(x)/d(x) around `x` using
/// automatic differentiation
template <typename X_t, typename CostOrResFunc>
auto AutoDiff(const X_t &x, const CostOrResFunc &cost_or_res_func) {
  ...
  const Index dims = If([&x], Dims != Dynamic, Dims, ptrait::dims(x));
  ...
  // Cols>}
  using DXJetType =
      std::conditional_t<is_userdef_type, Vector<Jet, Dims>, std::nullptr_t>;
  XJetType x_jet;
  ...
  if constexpr (is_userdef_type) { // X is user defined object
    ...
    for (Index i = 0; i < dims; ++i) {
      ...
      if constexpr (Dims == Dynamic)
        dx_jet[i].v = Vector<Scalar, Dynamic>::Zero(dims);
      dx_jet[i].v[i] = 1;
    }
    // dx_jet is constant
    ...
    for (int c = 0; c < x.cols(); ++c) {
      for (int r = 0; r < x.rows(); ++r) {
        ...
        if constexpr (Dims == Dynamic)
          x_jet(r, c).v = Vector<Scalar, Dims>::Zero(dims);
        x_jet(r, c).v[i] = 1;
      }
    }
  }
  ...
  if constexpr (!traits::is_matrix_or_array_v<ResType>) { // One residual
    ...
    const Index res_dims = If([&res], ResDims != Dynamic, ResDims, res.size());
    ...
    if constexpr (traits::is_matrix_or_array_v<ResType>) {
      if constexpr (ResType::ColsAtCompileTime !=
                    1) { // Matrix or Vector with dynamic size
...
auto CalculateJac(const X_t &x, const CostOrResFunc &cost_func) {
  const auto &[res, J] = AutoDiff(x, cost_func);
  return J;
}
