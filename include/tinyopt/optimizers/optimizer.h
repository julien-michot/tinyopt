...
public:
  using Options = std::conditional_t<std::is_null_pointer_v<_Options>,
                                     DefaultOptions, _Options>;

  Optimizer(const Options &_options = {})
      : options_{_options}, solver_(_options.solver) {}
    ...
    try {
      ...
      if (options_.log.enable) {
        ...
        if constexpr (std::is_base_of_v<typename SolverType::Options, Options2>)
          if (options_.save.H)
            num_hessians++;
        TINYOPT_LOG("Failed to allocate {} Hessian(s) of size {}x{}, "
                    "mem:{}GB, maybe use a SparseMatrix?",
                    num_hessians, dims, dims,
                    1e-9f * static_cast<float>(dims * dims * sizeof(Scalar)));
      }
  ...
  OutputType Optimize(X_t &x, const CostOrAccFunc &cost_or_acc,
                      int max_iters = -1) {
    ...
    if constexpr (std::is_invocable_v<CostOrAccFunc, const X_t &>) {
      ...
      if constexpr (std::is_invocable_v<CostOrAccFunc, const XJetType &>) {
        ...
        return tinyopt::OptimizeWithAutoDiff<kIsNLLS>(x, cost_or_acc, optimize,
                                                      options_);
      }
#else
      if constexpr (0) {
   ...
   */
  template <typename X_t, typename CostOrAccFunc>
  OutputType operator()(X_t &x, const CostOrAccFunc &cost_or_acc,
                        int max_iters = -1) {
  ...
  OutputType OptimizeAcc(X_t &x, const AccFunc &acc, int max_iters = -1) {
    ...
    out.start_time = tic();
    if (max_iters < 0)
      max_iters = options_.max_iters;
    max_iters++; // +1 to potentially roll-back
    if (options_.check_final_cost)
      max_iters++; // one last time to check the final error
    ...
    if constexpr (!kNoCopyX)
      best_x = new X_t(x); // using the copy constructor
    ...
    for (int iter = 0; iter < max_iters; ++iter) {
      ...
      if (success) { // Great, let's keep the good work
        ...
        // lower
        if (options_.check_final_cost && iter + 1 == max_iters)
          eval_only = true;
        ...
        eval_only =
            last_was_success == false; // No need to build the linear system
      ...
      if (options_.max_duration_ms > 0 &&
          out.duration_ms > options_.max_duration_ms) {
    ...
    if (options_.log.enable && out.stop_reason != StopReason::kNone)
      TINYOPT_LOG(
          "{}, cost: [{}]", StopReasonDescription(out, options_),
  ...
  /// decreased error)
  template <typename X_t, typename AccFunc>
  std::pair<bool, std::optional<Vector<Scalar, Dims>>>
  Step(X_t &x, const AccFunc &acc, OutputType &out) {
    const auto iter = out.num_iters;
    std::pair<bool, std::optional<Vector<Scalar, Dims>>> status{false,
                                                                std::nullopt};

    // Set start time if not set already
    const auto t = tic();
    if (out.start_time == TimePoint::min())
      out.start_time = t;
    ...
    const uint8_t max_tries =
        options_.max_consec_failures > 0
            ? std::max<uint8_t>(1, options_.max_consec_failures)
            : 255;
    for (; out.num_consec_failures <= max_tries;) {
      ...
      if (solver_failed) { // Failure
        ...
        if (cost.num_resisuals == 0) {
          if (options_.log.enable)
            TINYOPT_LOG("❌ #{}: No residuals, stopping", iter);
          out.stop_reason = StopReason::kSkipped;
          return status;
        } else if (std::isnan(cost.cost) ||
                   std::isinf(cost.cost)) { // Check for NaNs and Inf
          if (options_.log.enable)
            TINYOPT_LOG("❌ #{}: NaN/Inf in error", iter);
    ...
    if (std::isnan(err) || std::isinf(err)) {
      if (options_.log.enable)
        TINYOPT_LOG("❌ #{}: NaN/Inf in error: ε:{}", iter, err);
      out.stop_reason = StopReason::kSystemHasNaNOrInf;
      return status;
    }
    ...
    const bool has_grad_norm2 = options_.min_grad_norm2 > 0.0f ||
                                options_.stop_callback ||
                                options_.stop_callback2;
    const double grad_norm2 =
        has_grad_norm2 ? solver_.GradientSquaredNorm() : 0.0;
    ...
    const double rel_derr =
        out.final_cost > FloatEpsilon<Scalar>() &&
    ...
    if (options_.log.enable) {
      ...
      if (options_.log.print_emoji)
        oss << (is_good_step ? (iter == 0 ? "ℹ️" : "✅") : "❌");
      oss << "#" << iter << " ";
      ...
      if (options_.log.print_dx)
        oss << TINYOPT_FORMAT_NS::format("δx:[{}] ", dx);
      ...
      // Print error
      oss << TINYOPT_FORMAT_NS::format(
          "{}:{:.4e} n:{} d{}:{:+.2e} r{}:{:+.1e} ", options_.log.e, err, nerr,
          options_.log.e, derr, options_.log.e, rel_derr);
      if (has_grad_norm2)
    ...
    if (is_good_step) { /* GOOD Step */
      ...
      if (options_.max_total_failures > 0 &&
          out.num_failures >= options_.max_total_failures) {
    ...
    else if (options_.min_rerr_dec > 0 && rel_derr > 0.0 &&
             rel_derr < options_.min_rerr_dec)
    ...
    else if (options_.min_grad_norm2 > 0 &&
             grad_norm2 < options_.min_grad_norm2)
      out.stop_reason = StopReason::kMinGradNorm;
    else if (options_.stop_callback &&
             options_.stop_callback(err, dx_norm2, grad_norm2))
